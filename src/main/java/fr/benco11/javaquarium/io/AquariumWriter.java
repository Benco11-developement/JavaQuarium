...
public class AquariumWriter extends BufferedWriter {
    ...
    public void writeAquarium(Aquarium aquarium) throws IOException {
        ...
        Set<Integer> rounds = new HashSet<>(aquarium.remainingLivingsToAdd()
                                                    .keySet());
        rounds.addAll(aquarium.remainingRemoveOptions()
                              .keySet());
        for(int round : rounds) {
            ...
            List<Living> toAdd = aquarium.remainingLivingsToAdd()
                                         .getOrDefault(round, new ArrayList<>());
            writeFishes(toAdd.stream()
                             .filter(Fish.class::isInstance)
                             .map(Fish.class::cast)
                             .toList());
            writeKelps(toAdd.stream()
            ...
            Pair<List<Options>, List<Options>> removeOptions = aquarium.remainingRemoveOptions()
                                                                       .getOrDefault(round, new Pair<>(new ArrayList<>(), new ArrayList<>()));
    ...
    public void writeFishes(List<Fish> fishes) throws IOException {
        for(Fish fish : fishes)
            write(fish.name() + ", " + Fish.Species.of(fish)
                                                   .species() + ", " +
                    fish.sex() + ", " + pluralInsert("an", fish.age()) + ", " + pluralInsert("pv", fish.pv()) + "\n");
    }
    ...
    public void writeKelps(List<Kelp> kelps) throws IOException {
        for(Map.Entry<Integer, Long> kelpsAmountByAge : kelps.stream()
                                                             .collect(groupingBy(Kelp::age, Collectors.counting()))
                                                             .entrySet())
            write(pluralInsert("algue", kelpsAmountByAge.getValue()) + " " + pluralInsert("an", kelpsAmountByAge.getKey()) + "\n");
    }
    ...
    public void writeRemoveFishes(List<Options> options) throws IOException {
        for(Options option : options)
            write("-poisson " + option.optionsMap()
                                      .entrySet()
                                      .stream()
                                      .filter(e -> e.getValue()
                                                    .isPresent())
    ...
    private String ifPresentPluralOr(Options option, Options.StandardOption id, String word,
            String prefix, String or) {
        return option.ifPresentOr(id, v -> prefix + pluralInsert(word, v), Integer.class,
                (or == null)
                ? word
                : or);
    }
